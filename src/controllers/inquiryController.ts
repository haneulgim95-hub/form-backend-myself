...
const getInquiryList = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.status(200).json({ message: "나의 문의 목록을 성공적으로 조회했습니다.", data: result });
    } catch (error) {
        console.error(error);
...
const getInquiryById = async (req: AuthRequest<{ inquiryId: string }>, res: Response) => {
    try {
        ...
        if (result.userId !== userId) {
            res.status(403).json("해당 문의글을 조회할 권한이 없습니다.");
            return;
        }
...
const createInquiry = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.status(500).json({ message: "문의글을 등록하는 중 서버 에러가 발생했습니다."});
        return;
    }
};
...
const updateInquiry = async (req: AuthRequest<{inquiryId: string}>, res: Response) => {
    try {
        ...
        if (isNaN(inquiryId)) {
            res.status(400).json({ message: "유효하지 않은 문의글ID 입니다." });
            return;
        }

        const { title, content }: InquiryInputType = req.body;

        const result = await inquiryService.updateInquiry(userId, inquiryId,title, content);
...
const deleteInquiry = async (req: AuthRequest<{inquiryId: number}>, res: Response) => {
    try{
        if (!req.user) {
            res.status(401).json({ message: "인증 정보가 유효하지 않습니다." });
            return;
